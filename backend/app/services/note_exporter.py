...
def build_note_export_response(
    *,
    ...
    export_body = _with_visible_export_metadata(title=title, item=item, body=body)

    if fmt in {"md", "markdown", "raw_md"}:
        return _stream_bytes(
            export_body.encode("utf-8"),
...
def _with_visible_export_metadata(*, title: str, item: WorkspaceItem, body: str) -> str:
    """为 Markdown 与 Obsidian 导出补齐可见标题、类型，避免只剩 frontmatter。"""
    clean_body = body.strip()
    type_label = _ITEM_TYPE_LABELS.get(item.type, item.type or "未知")
    type_line = f"> 类型：{type_label}"

    if re.search(r"^>\s*类型[：:]", clean_body, flags=re.MULTILINE):
        return clean_body + "\n"

    if _title_from_body(clean_body):
        return re.sub(
            r"^(#\s+.+\n)",
...
def _build_pptx(*, title: str, safe_title: str, item: WorkspaceItem, body: str) -> StreamingResponse:
    ...
    def add_footer(slide, index: int) -> None:
        add_text(slide, Inches(0.62), Inches(7.05), Inches(2.5), Inches(0.22), "NoteBi Notes", size=8, color=muted)
        add_text(
            slide,
            Inches(11.55),
            Inches(7.05),
