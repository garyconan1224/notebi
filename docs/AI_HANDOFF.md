...
## 当前执行指针（2026-08-08，说话人精度 P0 合入 + P1 评测收尾 + 多人聚类修复）
...
- **当前 HEAD**：`222e446`（说话人 P0 两提交 `9d12a1a` / `222e446` 已在分支上并完成真实素材验证）。多人聚类修复未提交（见下）。
- **本轮内容**：说话人归属准确率 P0 完成并合入；P1（Community-1 vs WeSpeaker 模型 A/B）评测完成，结论是**保持 WeSpeaker，不切换默认管线**；多人场景 WeSpeaker 自动推断误拆（4→3）已用最小簇占比过滤修复。
...
- **已知边界**：0.65 阈值在 60-64% 覆盖区间偏保守（2 段被清标签）。

### 多人聚类修复（未提交）

- **根因**：WeSpeaker 自动推断（silhouette 选 k）在长音频上把同一说话人的声音变化误拆成额外的人。4.9 会议完整 83 分钟自动推断出 4 人，实际 3 人；多拆的 SPEAKER_00 全是 0.9-6s 短碎片（总占比仅 4.3%）。已存盘结果 4 人与此一致（同源误拆）。
- **修复**：`shared/wespeaker_diarizer.py` 新增 `_merge_small_clusters()`——自动推断（`num_speakers=None`）路径下，把总时长占比 <8% 的碎簇按时间最近邻并入主说话人。用户显式指定人数时跳过。
- **验证**（真实 embeddings）：4.9 会议完整 k 4→3 ✓；十字路口/姚顺宇双人 k 保持 2 ✓；密集 3 人窗口（4020-4260s）基线即对，A 不动 ✓；阈值 6-15% 稳定，20% 超调。单测 64 passed。
