...
## 当前执行指针（2026-08-04，Q1–Q8 收尾完成）

- **当前分支**：`codex/continue-product-redesign`
- **当前 HEAD**：本文件所在提交，以 `git log` 为准。
- **本轮计划**：`docs/plans/2026-08-02-product-revision.md`。Q1–Q7 已落地；Q8 CrisperWhisper 阶段 A POC 已完成，阶段 B 真实基准实测后发现中文长音频乱码（CrisperWhisper 库对非拉丁语适配缺陷，上游无修复），**评估后决定不纳入正式引擎，删除全部 Crisper 相关代码**。
...
- Q8 CrisperWhisper：阶段 B 真实基准已做（中文 60s 素材实测），确认库对中文长音频有乱码缺陷、上游无修复，决定不纳入正式引擎并删除全部相关代码。若上游日后修复，可重新评估。
...
Q1–Q8 已全部收尾。CrisperWhisper 因中文长音频不可用被放弃，不进入正式引擎；真实素材、真实 Vault、烧录视频和浏览器多视口像素级验收仍是后续验证边界。
