...
### 3.1 用户明确指定 Claude Code 的执行交接

当用户在当前任务明确要求 Claude Code 执行时，允许采用“Codex 制定计划与独立复验，Claude Code 实现”的一次性协作链；这不是默认执行方式。

1. Codex 先提供带基线、允许文件、禁止项、验收命令、停点和回报格式的可执行计划。
2. 用户可自行选择 Claude Code 的底层模型（例如 DeepSeek V4 Flash；必要时 Qwen 3.8）。模型别名或界面显示不是实际模型身份的证明，执行报告必须如实记录会话可见的配置，不得虚构署名或 commit trailer。
3. 有无关未提交改动时，Claude Code 必须在用户指定的干净 worktree 执行；不得修改、暂存、提交、清理或覆盖原工作区已有文件。
4. Claude Code 只执行计划中已确认的批次，不重新决定产品语义，不扩大文件范围，不修改本规则或计划文件；遇到依赖、模型下载、凭据、接口或数据契约偏差立即停下报告。
5. Claude Code 回报完整 diff、实际修改文件、测试命令与退出码、未验证项和 commit hash（如有）。Codex 必须在独立检查后才能批准下一批，不能把执行器口头结论当验收事实。

### 3.2 Sol / Luna Max 分工

本项目允许采用“Sol 当包工头，Luna Max 当工人”的串行协作方式，但最终责任仍由 Codex Sol 承担：

1. **Sol**：负责读取真实代码与运行证据、产品/架构决策、拆分边界、指定允许文件和验收命令、审查 diff、复跑测试与最终放行。
2. **Luna Max**：只接目标、文件范围和验收都明确的实现、修复、测试或机械性重构；不得重新决定产品语义、扩大范围、安装依赖、修改数据库、提交、合并或 push。
3. 项目级 Luna 配置优先放在 `.codex/agents/`；个人通用配置可放在 `~/.codex/agents/`。每次委托都必须重新说明允许文件、禁止项、停点和回报格式，不能只依赖模型名称。
4. Claude Code 使用 DeepSeek V4 Flash 时也按 Luna Worker 的边界执行：只做用户当前任务明确授权的有界子任务，外部结果必须由 Sol 用当前 Git、diff 和测试独立复验。
5. Open Design 只负责安装器、页面、交互和动效的设计/评审；它生成的原型不是生产代码，不能绕过依赖授权、代码审查与实机验收。

### 3.3 桌面安装器首次运行铁律

全平台桌面发行统一遵守以下产品契约：

1. 安装器必须让用户分别选择**应用安装目录**与**模型下载/存储目录**；两者独立保存、独立检查空间和写权限。
2. 用户确认模型清单和许可证后，安装器先下载或导入本次选择的必需模型，支持断点续传，并按版本、大小与 SHA256 完整校验。
3. 模型校验通过后再完成应用 runtime、FFmpeg、后端 sidecar 和前端资源安装；任一步失败都保留可复用成果并提供重试、改路径和日志入口。
4. 全部文件就绪后才启动本地后端；只有 `/health` 成功且前端资源可读取时，才启用“打开 NoteBi”并打开桌面窗口或页面。
5. 下载未完成、空间不足、无写权限、SHA256 不符或健康检查失败时，禁止打开残缺应用，不以成功页或自动跳转掩盖错误。
6. 离线包走“导入已有模型包 → manifest 校验 → 安装 → `/health`”同一状态机，明确禁止后台联网补包。
7. 安装器不得复制用户已有 `.local/settings.json`、API key、笔记数据库或模型服务凭据到发行资产；日志默认脱敏。
