...
1. NSIS / 系统包管理器负责应用安装目录；模型目录在安装后的“设置 → 本地模型”中独立选择和保存。
2. 安装阶段不得下载模型，也不得因模型未下载阻止首次打开；模型下载只能由用户在设置页明确点击触发。
3. 安装器完成 application runtime、FFmpeg、后端 sidecar 和前端资源；任一步失败都提供重试和日志入口。
4. 全部应用文件就绪后才启动本地后端；只有 `/health` 成功且前端资源可读取时，才进入 NoteBi 主页面。
5. 应用资源缺失、无写权限或健康检查失败时，禁止打开残缺应用，不以成功页或自动跳转掩盖错误。
6. 设置页模型下载必须显示来源、体积、进度、错误和缓存目录；下载完成后由用户选择是否激活，不在后台静默补包。
